...
def build_events(strategy: Strategy, *, target_resource: str) -> List[Dict[str, Any]]:
    """
    Toy but structured event generation:
    - device_hop: uses many device IDs (else one)
    - country_hop: rotates countries (else US)
    - ip_rotation: rotates IPs (else one)
    """
...
def main() -> None:
    ...
    # Baseline model + policy (public-safe)
    ...
    # Same tiny synthetic training schema as your adversarial_loop:
    # features vector order in vectorize(): [event_count, unique_ips, unique_devices, unique_countries]
    X_train = [
        [6, 1, 1, 1],   # normal
        [6, 2, 1, 1],   # some IP rotation
        [6, 6, 3, 1],   # device hopping
        [6, 6, 6, 3],   # device+country hopping
        [12, 12, 6, 3], # higher volume
    ]
    y_train = [0, 0, 1, 1, 1]
    ...
    for idx, strat in enumerate(strategies, start=1):
        ...
        # update trust across the episode timeline
    ...
    # rank: decision severity → ring_score → risk_score → alerts
